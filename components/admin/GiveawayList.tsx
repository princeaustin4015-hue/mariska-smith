...
export default function GiveawayList({ onEdit, refreshTrigger }: GiveawayListProps) {
  const toast = useToast()
  const { data: items, loading, error, refetch } = useAdminData<GiveawayItem>({
    endpoint: '/api/giveaways',
    refreshTrigger,
  })
  ...
  const handleDelete = async (item: GiveawayItem) => {
    ...
    if (confirmed) {
      await deleteMutation({
        endpoint: `/api/giveaways/${item.id}`,
        method: 'DELETE',
      })
    }
  }
